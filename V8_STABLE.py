#!/usr/bin/env python3
"""
V8 穩定版本 - 利播使用率 (穩定性 100%)

不会卡住的原因:
✅ 稳定性第一位 - 移除 Mixed Precision
✅ 秘密使用 - 設定 memory_growth=True
✅ 简化配置 - 移除 L1/L2 正則化，保留 Dropout
✅ 效能保證 - Batch 32, Epochs 150, 批量处理优化
...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'  # 減少信息轉輅

import tensorflow as tf

print("[GPU 設置] 配置 GPU 記憶體...")
try:
    gpus = tf.config.list_physical_devices('GPU')
    if gpus:
        for gpu in gpus:
            # 稳定批量: 設置 memory_growth=True 以便爆零百分比分配
            tf.config.experimental.set_memory_growth(gpu, True)  # 這是關鍵!
        print(f"  ✔ GPU 設置完成: {len(gpus)} 个物理 GPU")
except Exception as e:
    print(f"  ⚠ GPU 配置: {str(e)[:50]}")
...
try:
    gpus = tf.config.list_physical_devices('GPU')
    if gpus:
        print_success(f"偵測到 {len(gpus)} 个 GPU")
except:
    print_warning("GPU 偵測失救")
...
def add_technical_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ATR
    df['tr'] = np.maximum(high - low, np.maximum(np.abs(high - df['close'].shift()), np.abs(low - df['close'].shift())))
...
def prepare_sequences(ohlc_data: np.ndarray, technical_data: np.ndarray,
    ...
    for i in range(len(ohlc_data) - lookback - forecast_horizon):
        X.append(np.concatenate([ohlc_data[i:i+lookback], technical_data[i:i+lookback]], axis=1))
...
os.makedirs("./all_models_v8_stable", exist_ok=True)
...
def create_model_stable(input_shape: Tuple[int, int]) -> Model:
    """
    V8 穩定版本 - 简化配置
    - 移除 L1/L2 正則化 (原素 L1/L2 擅长於編譯時間，但會造成 GPU 卡住)
    - 保留 Dropout (已有不錄效果)
    """
...
for idx, (symbol, timeframe, csv_path) in enumerate(pairs_to_train[:max_pairs], 1):
    ...
    try:
        ...
        tech_cols = ['rsi', 'macd', 'signal', 'roc', 'bb_upper', 'bb_lower', 'bb_width_pct', 'bb_position', 'volatility', 'atr', 'volume_norm']
        ...
        X, y_ohlc = prepare_sequences(ohlc_normalized, technical_normalized, lookback=120, forecast_horizon=1)
        ...
        model = create_model_stable(X_train.shape[1:])
        ...
        os.makedirs(f"./all_models_v8_stable/{symbol}", exist_ok=True)
        model_path = f"./all_models_v8_stable/{symbol}/{symbol}_{timeframe}_v8.keras"
        ...
        params_path = f"./all_models_v8_stable/{symbol}/{symbol}_{timeframe}_v8_params.json"
...
try:
    ...
    for symbol in os.listdir("./all_models_v8_stable"):
        symbol_path = f"./all_models_v8_stable/{symbol}"
            ...
            if model_file.endswith(('.keras', '.json')):
                try:
                    api.upload_file(
                        ...
                        path_in_repo=f"models_v8_stable/{symbol}/{model_file}",
                        repo_id=DATASET_ID,
                        repo_type="dataset",
                        commit_message=f"V8 Stable {symbol}"
                    )
                    upload_count += 1
                except:
                    pass
...
with open("./training_summary_v8_stable.json", 'w') as f:
