...
for pkg in ['tensorflow>=2.13.0', 'xgboost>=2.0.0', 'lightgbm>=4.0.0', 'catboost>=1.2', 'datasets>=2.14.0', 'ta>=0.10.2', 'scikit-learn']:
    subprocess.check_call([sys.executable, '-m', 'pip', 'install', '-q', pkg])

print('[EXECUTE] Starting V9 Ensemble training...')
print('='*80)
...
class PipelineV3:
    ...
    def step3(self, data):
        print('\n' + '='*80 + '\n[STEP 3/6] INDICATORS + VOLATILITY FEATURES\n' + '='*80)
        d = data.copy()
        ...
        print('[INFO] Volatility Features')
        d['vol_10'] = d['close'].pct_change().rolling(10).std()
        d['vol_20'] = d['close'].pct_change().rolling(20).std()
        d['vol_50'] = d['close'].pct_change().rolling(50).std()
        d['hl_std'] = (d['high'] - d['low']).rolling(20).std()
        d['close_change_vol'] = d['close'].pct_change().rolling(10).std()
        d['parkinson_vol'] = np.sqrt(np.log(d['high'] / (d['low'] + 1e-10)) ** 2 / (4 * np.log(2))).rolling(10).mean()
        d['atr_ratio'] = d['atr'] / d['close']
        d['volume_volatility'] = d['volume'].rolling(20).std() / (d['volume_ma'] + 1e-10)
    ...
    def step4(self, data, seq_len=60, future_steps=10):
        ...
        yv = data['close'].pct_change().rolling(future_steps).std().values
    ...
    def step6a(self, Xt, Xv, Xte, yvt, yvv, yvte):
        print('\n' + '='*80 + '\n[STEP 6a/6] VOLATILITY ENSEMBLE\n' + '='*80)
        Xt2 = Xt.reshape(Xt.shape[0], -1)
        Xv2 = Xv.reshape(Xv.shape[0], -1)
        Xte2 = Xte.reshape(Xte.shape[0], -1)
        
        yvt_scaled = self.scaler_vol.fit_transform(yvt.reshape(-1, 1)).flatten()
        yvv_scaled = self.scaler_vol.transform(yvv.reshape(-1, 1)).flatten()
        yvte_scaled = self.scaler_vol.transform(yvte.reshape(-1, 1)).flatten()
        
        print('[INFO] Training XGBoost...')
        m1 = xgb.XGBRegressor(n_estimators=300, max_depth=8, learning_rate=0.05, 
                              subsample=0.9, colsample_bytree=0.9, reg_alpha=0.1, 
                              reg_lambda=1.0, random_state=42, verbosity=0)
        m1.fit(Xt2, yvt_scaled, eval_set=[(Xv2, yvv_scaled)], verbose=False)
        yp1 = m1.predict(Xte2)
        ...
        m3 = CatBoostRegressor(iterations=300, depth=8, learning_rate=0.05,
                               l2_leaf_reg=1.0, random_state=42, verbose=False)
        m3.fit(Xt2, yvt_scaled, eval_set=[(Xv2, yvv_scaled)])
        yp3 = m3.predict(Xte2)
        
        print('[INFO] Ensembling (0.4*XGB + 0.3*LGB + 0.3*CB)...')
        yp_ensemble = 0.4 * yp1 + 0.3 * yp2 + 0.3 * yp3
        yp_ensemble = self.scaler_vol.inverse_transform(yp_ensemble.reshape(-1, 1)).flatten()
        
        rmse = np.sqrt(mean_squared_error(yvte, yp_ensemble))
        mape = mean_absolute_percentage_error(yvte, yp_ensemble)
        r2 = r2_score(yvte, yp_ensemble)
        
        print(f'[INFO] Ensemble RMSE: {rmse:.8f}, MAPE: {mape:.4f}, R2: {r2:.4f}')
        print(f'[INFO] Previous R2: 0.5755 → New R2: {r2:.4f} (改善: {(r2-0.5755)*100:.2f}%)')
        
        m1.save_model('volatility_xgb_v3.json')
        m2.booster_.save_model('volatility_lgb_v3.txt')
        m3.save_model('volatility_cb_v3.cbm')
        
        self.results['volatility_ensemble'] = {
            'xgb_rmse': float(np.sqrt(mean_squared_error(yvte, self.scaler_vol.inverse_transform(yp1.reshape(-1, 1)).flatten()))),
            'lgb_rmse': float(np.sqrt(mean_squared_error(yvte, self.scaler_vol.inverse_transform(yp2.reshape(-1, 1)).flatten()))),
            'cb_rmse': float(np.sqrt(mean_squared_error(yvte, self.scaler_vol.inverse_transform(yp3.reshape(-1, 1)).flatten()))),
    ...
    def step7(self):
        ...
        rd = {'timestamp': datetime.now().isoformat(), 'pair': 'BTCUSDT', 'timeframe': '1h', 
              'version': 'v9_ensemble', 'models': self.results, 
              'note': 'Volatility: Ensemble (XGBoost+LightGBM+CatBoost), Price: LSTM'}
        with open('v9_results_ensemble.json', 'w') as f:
    ...
    def run(self):
        ...
        self.step7()
        print('\n' + '='*80)
        print('V9 ENSEMBLE TRAINING COMPLETED')
        print('='*80)
