...
Fixed version with TA library compatibility
"""
...
class V9TrainingPipeline:
    ...
    def step_3_calculate_indicators(self, data):
        ...
        try:
            ...
            self.log('Moving averages completed')
            ...
            self.log('Volume indicators completed')
            ...
            self.log('Price features completed')
            
            self.log('Filling NaN values')
            data_ind = data_ind.fillna(method='bfill').fillna(method='ffill')
            
            total_features = len([c for c in data_ind.columns if c not in ['open_time', 'close_time']])
    ...
    def step_6b_train_volatility_model(self, X_train, X_val, X_test,
        ...
        self.log('Preparing data for XGBoost')
