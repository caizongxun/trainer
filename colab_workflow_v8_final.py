...
def create_seq2seq_v8_model(input_shape: Tuple[int, int], forecast_horizon: int = 10) -> Model:
    ...
    model.compile(
        ...
        metrics={'ohlc_output': ['mae'], 'volatility_output': ['mae']}  # 修警：metrics 也是字典！
    )
            ...
            if model_file.endswith(('.keras', '.json')):
                try:
                    api.upload_file(
                        ...
                        path_in_repo=f"models_v8_final/{symbol}/{model_file}",
                        repo_id=DATASET_ID,
                        repo_type="dataset",
                        commit_message=f"Upload V8 Final {symbol} {model_file}"
                    )
                    upload_count += 1
