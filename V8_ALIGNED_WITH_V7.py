...
os.makedirs("./all_models_v8_v7aligned", exist_ok=True)
...
def create_seq2seq_v8_v7aligned(input_shape: Tuple[int, int], forecast_horizon: int = 10) -> Model:
    """
    V8 完全對標 V7 的模型架構
    
    Encoder: 3 層 BiLSTM (128->64->32) + LayerNorm
    Decoder: 1 層 LSTM (64) + LayerNorm
    輸出: 3 個任務 (OHLC + BB + Volatility)
    """
    lookback, n_features = input_shape
    
    inputs = Input(shape=(lookback, n_features), name='encoder_input')
    
    # Encoder - 3 層 BiLSTM (完全對標 V7)
    ...
    # 編譯 - 完全對標 V7 參數
    model.compile(
        optimizer=Adam(learning_rate=0.0005),  # V7: 0.0005
        loss={
            'ohlc_output': 'mse',
...
for idx, (symbol, timeframe, csv_path) in enumerate(pairs_to_train[:max_pairs], 1):
    ...
    try:
        ...
        # 序列準備 - 60 lookback (V7 標準)
        X, y_ohlc, y_bb, y_vol = prepare_sequences_v8(
        ...
        model = create_seq2seq_v8_v7aligned(X_train.shape[1:], forecast_horizon=10)
        
        # V7 標準的 Early Stopping 參數
        callbacks = [
        ...
        os.makedirs(f"./all_models_v8_v7aligned/{symbol}", exist_ok=True)
        model_path = f"./all_models_v8_v7aligned/{symbol}/{symbol}_{timeframe}_v8.keras"
        ...
        params_path = f"./all_models_v8_v7aligned/{symbol}/{symbol}_{timeframe}_v8_params.json"
...
try:
    ...
    for symbol in os.listdir("./all_models_v8_v7aligned"):
        symbol_path = f"./all_models_v8_v7aligned/{symbol}"
            ...
            if model_file.endswith(('.keras', '.json')):
                try:
                    api.upload_file(
                        ...
                        repo_id=DATASET_ID,
                        repo_type="dataset",
                        commit_message=f"V8 V7-Aligned {symbol}"
                    )
                    upload_count += 1
                except:
                    pass
...
with open("./training_summary_v8.json", 'w') as f:
