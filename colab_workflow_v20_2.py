#!/usr/bin/env python3
"""colab_workflow_v20_2.py

V20.2 "The Trend Surfer" (Trend-Specific Evolution)

This script focuses EXCLUSIVELY on evolving formulas for:
- Task B: Trend Start (Breakout)
- Task C: Trend End (Climax)

Features:
- V20 Core (Rich Indicators, Bloat Control, GPU Support)
- Specialized reporting for Trend strategies.

Run on Colab (GPU):
!pip install deap cupy-cuda12x && curl -s https://raw.githubusercontent.com/caizongxun/trainer/main/colab_workflow_v20_2.py | python3 - \
...
# ------------------------------
# 3. Labeling Targets (Trend Only)
# ------------------------------
def label_targets(df: pd.DataFrame) -> pd.DataFrame:
    lookback = 12
    # Task B: Trend Start (Regime switch 0->1)
    ...
    # Task C: Trend End (Pivot High in Trend)
...
def main():
    ...
    GLOBAL_TARGET = _maybe_to_xp(df["target_trend_start"].values.astype(np.int32), args.use_gpu, XP.int32)
    
    pop = toolbox.population(n=args.pop_size)
    hof = tools.HallOfFame(1)
    stats = tools.Statistics(lambda ind: ind.fitness.values)
    stats.register("max", np.max)
    algorithms_eaSimple(pop, toolbox, 0.5, 0.2, args.max_gens, stats, hof, True, args.patience, args.min_delta)
    best_start = hof[0]
    print(f"[Trend Start] Best: {best_start} (F1: {best_start.fitness.values[0]:.6f})")

    # 2. Trend End
    print(f"\n--- Evolving Trend End (Climax) ---")
    GLOBAL_TARGET = _maybe_to_xp(df["target_trend_end"].values.astype(np.int32), args.use_gpu, XP.int32)
    ...
    algorithms_eaSimple(pop, toolbox, 0.5, 0.2, args.max_gens, stats, hof, True, args.patience, args.min_delta)
    best_end = hof[0]
    print(f"[Trend End] Best: {best_end} (F1: {best_end.fitness.values[0]:.6f})")

    out_dir = f"./all_models/models_v20_2/{args.symbol}"
    _safe_mkdir(out_dir)
    with open(os.path.join(out_dir, "trend_report.txt"), "w") as f:
        f.write(f"Task: Trend Start\nFormula: {best_start}\nScore: {best_start.fitness.values[0]:.6f}\n\n")
        f.write(f"Task: Trend End\nFormula: {best_end}\nScore: {best_end.fitness.values[0]:.6f}\n")
