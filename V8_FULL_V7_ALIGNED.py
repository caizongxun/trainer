...
修正項目 (P0 級別):
✅ Lookback: 60 → 120 根 K 棒
✅ 特徵維度: 12 → 14 維 (加入 ROC、BB_Position、Volume_Norm)
✅ 正常化: StandardScaler → MinMaxScaler (保留物理約束)
✅ Epochs: 100 → 200

修正項目 (P1 級別):
✅ 編碼器: 3 層 → 4 層 BiLSTM (256→128→64→32)
✅ 梯度裁剪: clipvalue=1.0
✅ L1/L2 正則化: 1e-5
✅ Dropout 遞進: 0.3→0.3→0.2→0.2→0.2→0.1
✅ min_lr: 1e-6 → 1e-7
✅ 預測目標: 10 根 → 1 根 (單步預測)

執行:
!rm -rf ~/.cache/tensorflow ~/.cache/keras ~/.cache/torch
!curl -s https://raw.githubusercontent.com/caizongxun/trainer/main/V8_FULL_V7_ALIGNED.py | python
"""
...
def add_technical_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # BB_Position - P0 修正: 加入 (Close 在 BB 內的百分比位置)
...
def prepare_sequences_v8_v7aligned(ohlc_data: np.ndarray, technical_data: np.ndarray,
                                  lookback: int = 120, forecast_horizon: int = 1) -> Tuple:
    """V7 對標序列準備 - 120 根 lookback，預測 1 根"""
    X, y_ohlc = [], []
...
print_success("數據準備函數已初始化 (14 維特徵，120 lookback)")

print_header("4/7", "數據下載")

os.makedirs("./data/klines_binance_us", exist_ok=True)
os.makedirs("./all_models_v8_v7aligned_full", exist_ok=True)
...
from sklearn.preprocessing import MinMaxScaler  # P0 修正: MinMaxScaler
...
from tensorflow.keras import regularizers  # P1 修正: L1/L2 正則化

def create_seq2seq_v8_full_v7aligned(input_shape: Tuple[int, int]) -> Model:
    """
    V8 完整版本 - 100% 對標 V7
    
    4 層 Bidirectional LSTM: 256→128→64→32
    + LayerNormalization 每層後
    + 進度式 Dropout: 0.3→0.3→0.2→0.2→0.2→0.1
    + L1/L2 正則化: 1e-5
    + 梯度裁剪: 1.0
    
    輸出: Open, Close, High, Low (加權 OHLC)
    """
    lookback, n_features = input_shape
    
    inputs = Input(shape=(lookback, n_features), name='encoder_input')
    
    # P1 修正: 4 層 BiLSTM (256→128→64→32)
    # Layer 1: 256 units
    ...
    # Layer 4: 32 units (return_sequences=False 準備用於 Dense)
    ...
    # 輸出層: Open, Close, High, Low
    outputs = Dense(4, name='ohlc_output')(x)  # 無激活函數，線性回歸
...
for idx, (symbol, timeframe, csv_path) in enumerate(pairs_to_train[:max_pairs], 1):
    ...
    try:
        ...
        if technical_data.shape[1] != 11:
            # 確保是 11 維 (OHLC 4 維 + 技術指標 11 維 = 15 維)
            print(f"⚠ 特徵維度異常: {technical_data.shape[1]}")
            continue
        
        # P0 修正: MinMaxScaler (0-1)
        ...
        # P0 修正: Lookback 120, Forecast 1
        X, y_ohlc = prepare_sequences_v8_v7aligned(
        ...
        model = create_seq2seq_v8_full_v7aligned(X_train.shape[1:])
        
        # P0 修正: Epochs 200, min_lr 1e-7
        callbacks = [
            EarlyStopping(monitor='val_loss', patience=20, restore_best_weights=True),
            ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=8, min_lr=1e-7)  # P1 修正: min_lr
        ]
        
        history = model.fit(
            X_train, y_ohlc_train,
            validation_data=(X_val, y_ohlc_val),
            epochs=200,      # P0 修正: 200
            batch_size=16,
            verbose=0,
            callbacks=callbacks
        )
        ...
        os.makedirs(f"./all_models_v8_v7aligned_full/{symbol}", exist_ok=True)
        model_path = f"./all_models_v8_v7aligned_full/{symbol}/{symbol}_{timeframe}_v8_full.keras"
        ...
        params = {
            ...
            'lookback': 120,
            'forecast_horizon': 1,
            'version': 'v8_full_v7_aligned'
        }
        
        params_path = f"./all_models_v8_v7aligned_full/{symbol}/{symbol}_{timeframe}_v8_full_params.json"
...
try:
    ...
    for symbol in os.listdir("./all_models_v8_v7aligned_full"):
        symbol_path = f"./all_models_v8_v7aligned_full/{symbol}"
            ...
            if model_file.endswith(('.keras', '.json')):
                try:
                    api.upload_file(
                        ...
                        path_in_repo=f"models_v8_full/{symbol}/{model_file}",
                        repo_id=DATASET_ID,
                        repo_type="dataset",
                        commit_message=f"V8 Full V7-Aligned {symbol}"
                    )
                    upload_count += 1
                except:
                    pass
...
summary = {
    ...
    "architecture": "4-layer BiLSTM (256→128→64→32) + L1/L2 Reg + LayerNorm + Gradient Clip",
    "v7_corrections": {
        "lookback": "120 (was 60)",
        "features": "14 dimensions (added ROC, BB_Position, Volume_Norm)",
        "normalizer": "MinMaxScaler (was StandardScaler)",
        "epochs": "200 (was 100)",
        "encoder_layers": "4 BiLSTM (was 3)",
        "gradient_clip": "1.0",
        "l1l2_regularization": "1e-5",
        "dropout_progression": "0.3→0.3→0.2→0.2→0.2→0.1",
        "min_lr": "1e-7 (was 1e-6)",
        "forecast_horizon": "1 (was 10, single-step prediction)"
    },
    "hyperparameters": {
        "learning_rate": 0.0005,
        "batch_size": 16,
        ...
        "lookback": 120,
        "forecast_horizon": 1,
        "dropout": [0.3, 0.3, 0.2, 0.2, 0.2, 0.1],
...
with open("./training_summary_v8_full.json", 'w') as f:
    json.dump(summary, f, indent=2, ensure_ascii=False)

print_success("摘要已保存")

print("\n" + "="*60)
print("V8 完整版 (100% 對標 V7) 訓練上傳完成")
