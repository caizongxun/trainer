...
优化：
- 例罗录它格化优化 (Logging normalization)
- 加快訓練 (Reduced epochs, early stopping)
- MAPE 等评估指標
...
def add_technical_indicators(df):
    ...
    df['bb_position'] = np.where(
        upper_bb == lower_bb,
        0.5,
        np.clip((close_prices - lower_bb) / (upper_bb - lower_bb), 0, 1)
    )
...
def create_model(lookback=60, future_steps=10):
    model = Sequential([
        LSTM(64, activation='relu', input_shape=(lookback, 9), return_sequences=True),  # 推减到 64
        Dropout(0.2),
        LSTM(32, activation='relu', return_sequences=False),  # 推减到 32
        Dropout(0.2),
        Dense(16, activation='relu'),  # 推减到 16
...
# 訓練前 20 個模型 - 推减至前 10 個以上快速
for idx, (symbol, timeframe, csv_path) in enumerate(pairs_to_train[:10], 1):  # 例罗录它谘上手！
    ...
    try:
        ...
        if csv_path and os.path.exists(csv_path):
            ...
            df = pd.DataFrame({
                'open': np.random.uniform(0.9, 1.1, 5000).cumprod() * 100,  # 推减数据量从 10000 到 5000
                'high': np.random.uniform(1.0, 1.15, 5000).cumprod() * 100,
                'low': np.random.uniform(0.85, 1.0, 5000).cumprod() * 100,
                'close': np.random.uniform(0.9, 1.1, 5000).cumprod() * 100,
        ...
        val_rmse = calculate_rmse(y_val, y_pred_val)
        ...
        print(f"       Train - Loss: {train_loss:.6f}, MAPE: {train_mape:.2f}%, RMSE: {train_rmse:.6f}")
        print(f"       Val   - Loss: {val_loss:.6f}, MAPE: {val_mape:.2f}%, RMSE: {val_rmse:.6f}")
        ...
        del model, X, y, X_train, y_train, X_val, y_val, df, y_pred_train, y_pred_val
        gc.collect()
...
summary = {
    ...
    "optimizations": {
        "model_size_reduced": "128->64, 64->32, 32->16",
        "epochs_reduced": "25->15",
        "early_stopping": "Enabled (patience=5)",
        "batch_size_increased": "16->32",
        "metrics_added": "MAPE, RMSE"
    }
}
...
if training_results:
    ...
    print(f"  平均 Train MAPE: {avg_train_mape:.2f}%")
    print(f"  平均 Val MAPE: {avg_val_mape:.2f}%")
