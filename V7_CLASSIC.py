...
- 資料來源: HF 資料集 (zongowo111/cpb-models)
...
try:
    ...
    os.system('rm -rf ./all_models_v7* 2>/dev/null')
    print_success("緩存已清理")
except:
    pass
...
try:
    ...
    if gpus:
        ...
        print_success(f"偵測到 {len(gpus)} 個 GPU")
except:
    pass
...
print_header("4/7", "從 HF 資料集下載 OHLC 資料")

os.makedirs("./data/klines_binance_us", exist_ok=True)
os.makedirs("./all_models_v7", exist_ok=True)
...
try:
    ...
    print_success(f"準備訓練 {min(40, len(pairs_to_train))} 個模型")
    
except Exception as e:
    print_error(f"下載失敗: {e}")
    pairs_to_train = [("BTCUSDT", "15m", None)]

print_header("5/7", "模型訓練 (V7 標準)")
...
for idx, (symbol, timeframe, csv_path) in enumerate(pairs_to_train[:max_pairs], 1):
    ...
    try:
        ...
        # 保存模型
        os.makedirs(f"./all_models_v7/{symbol}", exist_ok=True)
        model_path = f"./all_models_v7/{symbol}/{symbol}_{timeframe}_v7.keras"
        ...
        params_path = f"./all_models_v7/{symbol}/{symbol}_{timeframe}_v7_params.json"
...
try:
    ...
    for symbol in os.listdir("./all_models_v7"):
        symbol_path = f"./all_models_v7/{symbol}"
            ...
            if model_file.endswith(('.keras', '.json')):
                try:
                    api.upload_file(
                        ...
                        path_in_repo=f"models_v7/{symbol}/{model_file}",
                        repo_id=DATASET_ID,
                        repo_type="dataset",
                        commit_message=f"V7 Classic {symbol}"
                    )
                    upload_count += 1
                except:
                    pass
...
with open("./training_summary_v7.json", 'w') as f:
    json.dump(summary, f, indent=2, ensure_ascii=False)

print_success("摘要已保存")

print("\n" + "="*60)
print("V7 Classic 訓練上傳完成")
