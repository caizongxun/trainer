#!/usr/bin/env python3
"""colab_workflow_v21.py

V21 "The Price Action Specialist" (Rich Features + Strict Bloat Control)

New Features:
1. Price Action: UpperShadow, LowerShadow, BodyHeight, Gap
2. Time: HourOfDay, DayOfWeek
3. Strict Bloat Control: Hard limits on tree depth/size to prevent Python recursion errors.

Run on Colab (GPU):
!pip install deap cupy-cuda12x && curl -s https://raw.githubusercontent.com/caizongxun/trainer/main/colab_workflow_v21.py | python3 - \
...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    d["day_of_week"] = d["open_time"].dt.dayofweek # 0=Mon, 6=Sun
...
def eval_formula(individual):
    # BLOAT CONTROL: Hard limit on nodes
...
def evolve_for_task(task_name, target_array, inputs, pop_size=50, gens=5, patience=0, min_delta=0.0, use_gpu=False):
    print(f"\n--- Evolving for Task: {task_name} ---")
    global GLOBAL_INPUTS, GLOBAL_TARGET
    
    # 這裡的 inputs 已經是 GPU array (如果 enabled)
...
def main():
    ...
    p.add_argument("--max_height", type=int, default=8) # Default tightened
    p.add_argument("--max_nodes", type=int, default=100) # Default tightened
    ...
    out_dir = f"./all_models/models_v21/{args.symbol}"
    _safe_mkdir(out_dir)
    report_file = os.path.join(out_dir, "alpha_factory_report.txt")
    
    with open(report_file, "w") as f:
        f.write(f"=== ALPHA FACTORY REPORT (V21) ({args.symbol}) ===\n")
        f.write(f"New Features: Price Action (Shadows, Body, Gap) + Time\n\n")
        ...
        # Task C
        target = _maybe_to_xp(df["target_trend_end"].values.astype(np.int32), args.use_gpu, XP.int32)
        best, score, meta = evolve_for_task("Trend End", target, inputs, args.pop_size, args.max_gens, args.patience, args.min_delta, args.use_gpu)
